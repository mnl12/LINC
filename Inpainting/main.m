function [imfi]= main(ju,refju,N,imn,s,mnc,iter,sigmaa,imna,mask)
dim=size(imn,1);
dims2=size(imn,2);
imfi=zeros(size(imn));
w=zeros(size(imn));
...
for rp=1:refju:size(imn,1)
    ...
    for cp=1:refju:size(imn,2)
        ...
        maskp=mask(rp:rp+s-1,cp:cp+s-1);
        refpa=imna(rp:rp+s-1,cp:cp+s-1);
        refpv=reshape(refp,[s*s,1]);
        refmask=reshape(maskp,[s*s,1]);
        ...
        for opr=sr:ju:er-s+1
            for opc=sc:ju:ec-s+1
                ...
                pma=mask(opr:opr+s-1,opc:opc+s-1);
                y=reshape(pa,[s*s,1]);
                ya=reshape(paa,[s*s,1]);
                ma=reshape(pma,[s*s,1]);
                Y1(:,dimc)=y;
                Y1a(:,dimc)=ya;
                Ma(:,dimc)=ma;
                pos1(:,dimc)=[opr;opc];
        ...
        Y=zeros(s*s,mnc+1);
        Ya=zeros(s*s,mnc+1);
        [Y,Ya,pos,Mas]=KNN(refpv,Y1,s,pos1,mnc,Y1a,Ma,iter,refmask);
        dimc2=size(Y,2);
        Y(:,dimc2+1)=refpv;
        Ya(:,dimc2+1)=refpva;
        Mas(:,dimc2+1)=refmask;
        pos(:,dimc2+1)=[rp;cp];
        %denoising
        [Yd wc]=lowrcovsimp2(Y,Ya,sigmaa,Mas,iter);
        clear Y;
        for i=1:size(Yd,2)
            cor1=pos(1,i);
            cor2=pos(2,i);
